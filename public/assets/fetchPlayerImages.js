...
async function downloadPlayerImage(id, name) {
    ...
    try {
        ...
        console.error(`   ⚠️ Failed to download image for ${name}: ${err.message}`);
        return false;
    }
}

async function downloadAllPlayers(ids, names) {
    for (let i = 0; i < ids.length; i++) {
        const id = ids[i];
        const name = names[i];
        
        await downloadPlayerImage(id, name);
...
async function playersFromTeam(teamId) {
    ...
    try {
        ...
        const resBody = res.body.players;
        const returnIds = resBody.map(item => item.player.id);
        const returnNames = resBody.map(item => item.player.name);
...
for (const team of teamIds) {
    ...
    const result = await playersFromTeam(team);
    ...
    const { returnIds, returnNames } = result;
    const convertedPlayerNames = returnNames.map(name => convertToFileSyntax(name));
    
    console.log(`📥 Downloading ${returnIds.length} players...`);
    await downloadAllPlayers(returnIds, convertedPlayerNames);
